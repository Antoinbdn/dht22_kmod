/* dht22km.c
 *
 * dht22km - Device driver for reading values from DHT22 temperature and
 *humidity sensor.
 *
 * Usage:
 *        Load driver: 	insmod ./dht22km.ko <optional variables>
 *				i.e.   	insmod ./dht22km.ko gpio_pin=2 format=3
 *
 *		  To read the values from the sensor: sudo cat /dev/dht22
 *
 *  author: Alvin Chien <alvinchien0624@gmail.com>
 *  Original author: Nigel Morton <nigel@ntpworld.co.uk>
 *
...
#define DHT22_DRIVER_NAME "dht22"
#define RBUF_LEN 256
#define SUCCESS 0
...
// according to the datasheet of bcm2835
#define GPIO_KERN_VIRT_ADDR ((void *)0xf2200000)

// module parameters
static int sense = 0;
static struct timeval lasttv = {0, 0};

// lock for setting interrupt
static spinlock_t lock;
// device semaphore
static struct semaphore dev_sema;

// Forward declarations
static int read_dht22(struct inode *, struct file *);
static int close_dht22(struct inode *, struct file *);
...
static int humidity_conv = 0;
static int temperature_conv = 0;
...
static struct file_operations fops = {
    .read = device_read, .open = read_dht22, .release = close_dht22};

static struct miscdevice dht22_miscdev = {
    .minor = MISC_DYNAMIC_MINOR, .name = DHT22_DRIVER_NAME, .fops = &fops,
};

// Possible valid GPIO pins
int valid_gpio_pins[] = {0,  1,  4,  8,  7,  9,  10, 11, 14,
...
static irqreturn_t irq_handler(int i, void *blah, struct pt_regs *regs) {
  ...
  if (sense != -1) {
    ...
    if ((signal == 0) & (started == 1)) {
      ...
      // Uncomment to log bits and durations - may affect performance and not be
      // accurate!
      ...
      // if(bytecount == 5)
      //	printk(KERN_INFO DHT22_DRIVER_NAME "Result: %d, %d, %d, %d,
...
static int setup_interrupts(void) {
  ...
  result = request_irq(gpio_irq, (irq_handler_t)irq_handler, 0,
                       DHT22_DRIVER_NAME, (void *)gpio);

  switch (result) {
  case -EBUSY:
    printk(KERN_ERR DHT22_DRIVER_NAME ": IRQ %d is busy\n", gpio_irq);
    return -EBUSY;
  case -EINVAL:
    printk(KERN_ERR DHT22_DRIVER_NAME ": Bad irq number or handler\n");
    return -EINVAL;
  default:
    printk(KERN_INFO DHT22_DRIVER_NAME ": Interrupt %04x obtained\n", gpio_irq);
    break;
  };
...
static int init_port(void) {
  if (gpio_request(gpio_pin, "output")) {
    printk("GPIO request faiure: %s\n");
    return -EBUSY;
  }

  gpio = (unsigned int *)GPIO_KERN_VIRT_ADDR;
  printk(KERN_INFO DHT22_DRIVER_NAME "gpio: %p\n", gpio);
  return 0;
}

static int __init dht22_init_module(void) {
  int result;
  int i;
  ...
  if (result != 1) {
    result = -EINVAL;
    printk(KERN_ERR DHT22_DRIVER_NAME ": invalid GPIO pin specified!\n");
    goto exit_rpi;
  }

  result = misc_register(&dht22_miscdev);

  if (result < 0) {
    printk(KERN_ALERT DHT22_DRIVER_NAME
           "Registering dht22 driver failed with %d\n",
           result);
    return result;
  }

  printk(KERN_INFO DHT22_DRIVER_NAME ": misc driver registered!\n");

  result = init_port();
  if (result < 0) {
    misc_deregister(&dht22_miscdev);
    goto exit_rpi;
  }
...
static void __exit dht22_exit_module(void) {
  // release gpio pin
  if (gpio != NULL) {
    gpio_free(gpio_pin);
    printk(DHT22_DRIVER_NAME ": cleaned up resources\n");
  }

  // Unregister the driver
  misc_deregister(&dht22_miscdev);
  printk(DHT22_DRIVER_NAME ": cleaned up module\n");
}

// Called when a process wants to read the dht22 "cat /dev/dht22"
static int read_dht22(struct inode *inode, struct file *file) {
  char result[3]; // To say if the result is trustworthy or not
  int retry = 0;

  if (down_trylock(&dev_sema))
    return -EBUSY;

  try_module_get(THIS_MODULE); // Increase use count

// Take data low for min 18mS to start up DHT22
// printk(KERN_INFO DHT22_DRIVER_NAME " Start setup (read_dht22)\n");
...
start_read:
  ...
  mdelay(20);                // DHT22 needs min 18mS to signal a startup
  ...
  if ((dht[0] + dht[1] + dht[2] + dht[3] == dht[4]) & (dht[4] > 0))
    ...
    clear_interrupts();
    mdelay(2100); // Can only read from sensor every 2 second so give it time to
                  // recover
    goto start_read;
  }
...
return_result:
  ...
  case 3:
    // for DHT22
    humidity_conv = dht[0] * 256 + dht[1];
    temperature_conv = (dht[2] & 0x7F) * 256 + dht[3];
    if (dht[2] & 0x80) {
      temperature_conv *= -1;
    }
    sprintf(msg, "Temperature: %d.%dC; Humidity: %d.%d%%\nResult:%s\n",
            temperature_conv / 10, temperature_conv % 10, humidity_conv / 10,
            humidity_conv % 10, result);
    break;
  }
  msg_Ptr = msg;
...
static int close_dht22(struct inode *inode, struct file *file) {
  ...
  clear_interrupts();
  printk(KERN_INFO DHT22_DRIVER_NAME ": Device release (close_dht22)\n");
...
static ssize_t device_read(struct file *filp, // see include/linux/fs.h
  ...
  while (length && *msg_Ptr) {
    ...
    // assignment won't work.  We have to use
    // put_user which copies data from the kernel data segment to the user data
    // segment.
...
module_init(dht22_init_module);
module_exit(dht22_exit_module);

MODULE_DESCRIPTION(
    "DHT22 temperature/humidity sendor driver for Raspberry Pi GPIO.");
MODULE_AUTHOR("Alvin Chien");
MODULE_LICENSE("GPL");
